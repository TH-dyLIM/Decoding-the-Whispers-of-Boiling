...
# 경로 설정
accuracy_path = os.path.join(save_folder, 'cnn_vgg16_accuracy.xlsx')
best_model_path = os.path.join(save_folder, 'cnn_vgg16_best_model.pth')
training_loss_path = os.path.join(save_folder, 'cnn_vgg16_training_loss.png')
confusion_matrix_path = os.path.join(save_folder, 'cnn_vgg16_confusion_matrix.png')
hyperparameters_path = os.path.join(save_folder, 'cnn_vgg16_hyperparameters.txt')
test_results_path = os.path.join(save_folder, 'cnn_vgg16_test_results.xlsx')
params_vs_score_path = os.path.join(save_folder, 'cnn_vgg16_params_vs_score.png')
layers_vs_score_path = os.path.join(save_folder, 'cnn_vgg16_layers_vs_score.png')
...
class CNN2DVGG16(nn.Module):
    def __init__(self, num_classes):
        super(CNN2DVGG16, self).__init__()
        self.model = vgg16(weights=VGG16_Weights.DEFAULT)
        self.model.classifier = nn.Sequential(*list(self.model.classifier.children())[:-1])  # Remove the last layer
...
class TorchModelWrapper(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        self.model = CNN2DVGG16(self.num_classes).to(device)
