...
save_folder = './Results_Flow_100%_Spectrogram'
...
"""
# 데이터셋 경로 설정
dataset_train = './dataset_stft_flow_train'
dataset_test = './dataset_stft_flow_test'
train_label = './dataset_stft_flow_train/labels.xlsx'
test_label = './dataset_stft_flow_test/labels.xlsx'

# 저장된 모델 경로
best_model_path = './Results_Pool_100%_Spectrogram/transformer_best_model.pth'
...
class TorchModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, num_classes, d_model, nhead, num_layers, learning_rate=0.0001,
                 num_epochs=30, batch_size=16, optimizer='adam', weight_initialization='default',
        ...
        self.pretrained_weights = pretrained_weights  # Add this line
        ...
        if self.pretrained_weights is not None:
            self.model.load_state_dict(torch.load(self.pretrained_weights, map_location=device))
        else:
            # Apply weight initialization only if not loading pre-trained weights
...
# 저장된 모델 로드
model = TorchModelWrapper(num_classes, d_model, nhead, num_layers, pretrained_weights=best_model_path)
