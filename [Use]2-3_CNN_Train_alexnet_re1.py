...
# 경로 설정
accuracy_path = os.path.join(save_folder, 'cnn_alexnet_300epoch_accuracy.xlsx')
best_model_path = os.path.join(save_folder, 'cnn_alexnet_300epoch_best_model.pth')
training_loss_path = os.path.join(save_folder, 'cnn_alexnet_300epoch_training_loss.png')
confusion_matrix_path = os.path.join(save_folder, 'cnn_alexnet_300epoch_confusion_matrix.png')
hyperparameters_path = os.path.join(save_folder, 'cnn_alexnet_300epoch_hyperparameters.txt')
test_results_path = os.path.join(save_folder, 'cnn_alexnet_300epoch_test_results.xlsx')
params_vs_score_path = os.path.join(save_folder, 'cnn_alexnet_300epoch_params_vs_score.png')
layers_vs_score_path = os.path.join(save_folder, 'cnn_alexnet_300epoch_layers_vs_score.png')
...
"""
# 데이터셋 경로 설정
dataset_train = './spectrograms/dataset_signal_pool_train'
dataset_test = './spectrograms/dataset_signal_pool_test'
train_label = './spectrograms/dataset_signal_pool_train/labels.xlsx'
test_label = './spectrograms/dataset_signal_pool_test/labels.xlsx'
"""
fold = 4
batch_size = 50
learning_rate = 0.001
...
class CNN2DAlexNet(nn.Module):
    def __init__(self, num_classes):
        super(CNN2DAlexNet, self).__init__()
        ...
        self.fc_heat_flux = nn.Linear(4096, 1)
        self.fc_heat_transfer_coefficient = nn.Linear(4096, 1)
        self.fc_boiling_regime = nn.Linear(4096, num_classes)
...
class TorchModelWrapper(BaseEstimator, ClassifierMixin):
    def __init__(self, num_classes, learning_rate=0.001, num_epochs=50, batch_size=16, optimizer='adam', weight_initialization='default'):
    ...
    def fit(self, X, y):
        self.model = CNN2DAlexNet(self.num_classes).to(device)
...
def custom_score(y_true, y_pred):
    ...
    # num_samples = y_pred.size // 3
    # if y_pred.size % 3 != 0:
    #    raise ValueError(f"y_pred size {y_pred.size} cannot be reshaped into (num_samples, 3)")

    # y_pred = y_pred.reshape(num_samples, 3)
