...
# 경로 설정
accuracy_path = os.path.join(save_folder, 'cnn_resnet_accuracy.xlsx')
best_model_path = os.path.join(save_folder, 'cnn_resnet_best_model.pth')
training_loss_path = os.path.join(save_folder, 'cnn_resnet_training_loss.png')
confusion_matrix_path = os.path.join(save_folder, 'cnn_resnet_confusion_matrix.png')
hyperparameters_path = os.path.join(save_folder, 'cnn_resnet_hyperparameters.txt')
test_results_path = os.path.join(save_folder, 'cnn_resnet_test_results.xlsx')
params_vs_score_path = os.path.join(save_folder, 'cnn_resnet_params_vs_score.png')
layers_vs_score_path = os.path.join(save_folder, 'cnn_resnet_layers_vs_score.png')
...
class CNN2DResNet(nn.Module):
    def __init__(self, num_classes):
        super(CNN2DResNet, self).__init__()
...
class TorchModelWrapper(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, y):
        self.model = CNN2DResNet(self.num_classes).to(device)
